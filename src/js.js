...
function buildJS(shouldMinify = true) {
    ...
    // If dist dir does not exist, create it
    if (!fs_1.default.existsSync(outputDir)) {
        fs_1.default.mkdirSync(outputDir, { recursive: true });
    }
    const files = (0, utils_1.findJSFiles)(jsDir);
    (0, concat_1.default)(files)
        .then((result) => {
        if (shouldMinify) {
            // Minify the concatenated result
            ...
            // Write the concatenated result without minification
            fs_1.default.writeFileSync(outputFile, result);
            console.log("JS concatenated successfully (no minification).");
        }
    })
        .catch((error) => {
...
function watchJS() {
    const buildJSDebounced = (0, lodash_debounce_1.default)(() => buildJS(false), 300);
    const watcher = chokidar_1.default.watch(jsDir, {
        ignored: path_1.default.join(cwd, "assets/**"),
        persistent: true,
    });
    watcher.on("add", buildJSDebounced).on("change", buildJSDebounced).on("unlink", buildJSDebounced);
